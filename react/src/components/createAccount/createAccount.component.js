...
class createAccount extends Component {
	...
	<Card.Body>
		<Card.Title className="cardTitleStyle">Создание аккаунта</Card.Title>
		<form class="flex-column" onSubmit = {this.submit}>
			...
			<form class="flex-row" onSubmit = {this.submit}>
				<div class="form-group pt-0 pl-2 pr-2">
					<label class="text-dark font-weight-bold">Вы присоединяетесь в качестве студента или преподователя?</label>
				...
				<div class="btn-toolbar" role="toolbar" aria-label="Toolbar with button groups">
					<div class="btn-group mr-2 pt-0 pl-2 pr-2" id="userStudent">
						<button type="button" class="btn btn-outline-secondary" name='user_type' value={'student'} onClick={this.onclick} >Студент</button>
					</div>
					<div class="btn-group mr-2 pt-0 pl-2 pr-2" id="userInstructor">
					<button type="button" class="btn btn-outline-secondary" name='user_type' value={'instructor'} onClick={this.onclick} >Преподователь</button>
					</div>
				</div>
			</form>
